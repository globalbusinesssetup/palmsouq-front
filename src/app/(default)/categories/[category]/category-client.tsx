...
const CategoryClient: React.FC<CategoryClientProps> = ({ products, category }) => {
      ...
      <main className="bg-[#FCFCFD]">
        <div className="container mx-auto px-4">
          <div className="w-full h-[150px] md:h-[180px] lg:h-[200px] bg-secondary rounded-md mt-6 relative overflow-hidden">
            <Image
              src={'https://images.unsplash.com/photo-1565688842882-e0b2693d349a?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'}
              fill
              alt="banner"
          ...
          <div className="grid xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-6 pt-4 pb-10">
            {products?.data?.length ? (
              products?.data?.map((product:any, i: number) => (
                <ProductCard key={`product_${i}`} data={product} category={category} />
              ))
            ) : (
