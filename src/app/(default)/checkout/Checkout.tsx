...
import { getAddress, getCart, getPayMethods, useGetUser } from '@/utils/api';
...
import { orderEncrypt, timezone } from '@/utils/helper';
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    setDefaultAddress(addresses?.data[0].state!);
  }, [defaultAddress, addresses, ordersData]);
  ...
  const handleUpdateCart = async () => {
    ...
    try {
      const res = await api.post('/cart/update-shipping', {
        ...
        selected_address: addresses?.data[0]?.id,
      });
    } catch (err) {
      console.log(err);
    }
